...
import com.wqz.echonetwork.entity.po.ArticleLike;
...
public class ArticleLikeMapper {
    ...
    public List<ArticleLike> findByUserId(Long userId) {
        return SqlUtil.queryList(
                "SELECT * FROM article_like WHERE user_id = ? ORDER BY create_time DESC",
                ArticleLike.class,
                userId
        );
    }
    ...
    public int deleteByArticleId(Long articleId) {
        return SqlUtil.update(
                "DELETE FROM article_like WHERE article_id = ?",
                articleId
        );
    }
}
